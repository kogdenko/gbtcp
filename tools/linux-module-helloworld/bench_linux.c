...
he_data_ready(struct sock *sk)
{
	...
	if (s == NULL) {
		...
		he_soqueue(s);
		D("data_ready: cpu=%d, port=%hu", smp_processor_id(),
		  s->hs_port);
	}
	read_unlock_bh(&sk->sk_callback_lock);
	if (data_ready != NULL) {
...
he_read(struct work_struct *w)
{
	...
	if (s->hs_so->sk->sk_state == TCP_ESTABLISHED) {
		...
		if (len > 0) {
			...
			D("send: cpu=%d, port=%hu", smp_processor_id(),
			  s->hs_port);
		}
	} else {
		he_sodel(s);
...
he_accept(struct work_struct *w)
{
	...
	while (1) {
		...
		sk = aso->sk;
		write_lock_bh(&sk->sk_callback_lock);
		rc = aso->ops->getname(aso, (struct sockaddr *)&sin,
				       sizeof(sin));
		if (rc < 0) {
			goto err1;
		}
...
he_init(void)
{
	...
	for (i = 0; i < ncpus; ++i) {
		...
		t->ht_slab = kmem_cache_create(slabnam, sizeof(struct he_so), 0,
...
module_init(he_init) module_exit(he_exit)
