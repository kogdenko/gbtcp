...
#include <kernel/subr.h>

#include <tools/common/gtt_subr.h>
#include <tools/common/gtt_worker.h>
...
static void on_event(struct gtt_worker *, int, const union connection *, short);
...
static int
x_socket(int domain, int type, int protocol)
{
	int rc;
	...
	if (rc == -1) {
		assert(errno);
		gtt_die(errno, "socket(domain=0x%x, type=0x%x) failed", domain,
			type);
	}
	return rc;
}
...
{
	gtt_errorf(errnum, "connect(%s:%hu) failed", inet_ntoa(addr->sin_addr),
		   ntohs(addr->sin_port));
}

static int
x_connect(int fd, struct sockaddr_in *addr)
{
	int rc;
...
static int
x_getsockopt(int sockfd, int level, int optname, void *optval,
	     socklen_t *optlen)
{
	int rc;
	...
	if (rc == -1) {
		assert(errno);
		rc = -errno;
		gtt_errorf(-rc, "getsokopt() failed");
	}
...
x_listen(int fd, int backlog)
{
	...
	if (rc == -1) {
		assert(errno);
		gtt_die(errno, "listen(fd=%d, backlog=%d) failed", fd, backlog);
	}
	return rc;
}

static int
x_bind(int fd, struct sockaddr_in *addr)
{
	int rc;
	...
	if (rc == -1) {
		assert(errno);
		gtt_die(errno, "bind(fd=%d, addr=%s, port=%hu) failed", fd,
...
x_accept4(int fd, int flags)
{
	...
	if (rc == -1) {
		...
		rc = -errno;
		if (errno != EAGAIN) {
			gtt_die(-rc, "accept4(fd=%d, 0x%x) failed", fd, flags);
		}
	}
	return rc;
}
...
static int
x_setsockopt(int fd, int level, int optname, const void *optval,
	     socklen_t optlen)
{
	int rc;
	...
	if (rc == -1) {
		assert(errno);
		rc = -errno;
		gtt_die(-rc, "setsockopt(fd=%d, %d, %d) failed", fd, level,
			optname);
	}
	return rc;
}
...
event_queue_create(void)
{
	...
	if (rc == -1) {
		assert(errno);
		rc = -errno;
		gtt_die(-rc, "epoll_create1() failed");
	}
	return rc;
}
...
event_queue_ctl(int eq_fd, int is_new, const union connection *cp, int write)
{
	...
	rc = epoll_ctl(eq_fd, is_new ? EPOLL_CTL_ADD : EPOLL_CTL_MOD,
		       cp->conn_fd, &event);
	if (rc == -1) {
		assert(errno);
		rc = -errno;
		gtt_die(-rc, "epoll_ctl(%s, events=0x%x, fd=%d) failed",
			is_new ? "EPOLL_CTL_ADD" : "EPOLL_CTL_MOD",
			event.events, cp->conn_fd);
	}
}

static void
event_queue_wait(struct gtt_worker *worker, int eq_fd, int to_ms)
{
	int i, n;
	short revents;
	...
	n = epoll_wait(eq_fd, events, GTT_ARRAY_SIZE(events), to_ms);
	for (i = 0; i < n; ++i) {
		e = events + i;
		revents = 0;
		assert(e->events);
...
event_queue_ctl(int eq_fd, int is_new, const union connection *cp, int write)
{
	...
	if (rc == -1) {
		assert(errno);
		rc = -errno;
		gtt_die(-rc, "kevent(EV_ADD, EVFILT_READ, fd=%d) failed",
			cp->conn_fd);
	}
}
...
event_queue_wait(struct gtt_worker *worker, int eq_fd, int to_ms)
{
	...
	n = kevent(eq_fd, NULL, 0, eventlist, GT_ARRAY_SIZE(eventlist), &ts);
	for (i = 0; i < n; ++i) {
		e = eventlist + i;
		revents = 0;
...
server(int eq_fd)
{
	...
	fd = x_socket(AF_INET, SOCK_STREAM | SOCK_NONBLOCK, 0);
	if (g_dflag) {
		set_so_debug(fd);
	}
	o = 1;
	x_setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &o, sizeof(o));
	x_setsockopt(fd, SOL_SOCKET, SO_REUSEPORT, &o, sizeof(o));
	x_bind(fd, &g_addr);
	x_listen(fd, 128);
	conn.conn_u64 = 0;
	conn.conn_fd = fd;
...
static void
client(struct gtt_worker *worker, int eq_fd)
{
	int rc, fd, write;
	union connection conn;
	...
	conn.conn_u64 = 0;
	conn.conn_fd = fd;
	rc = x_connect(fd, &g_addr);
	if (rc == 0) {
		rc = gtt_write_record(fd, g_http, g_http_len);
		if (rc < 0) {
			close(fd);
			return;
		}
...
static void
clientn(struct gtt_worker *worker, int eq_fd)
{
...
static void
on_event(struct gtt_worker *worker, int eq_fd, const union connection *cp,
	 short revents)
{
	...
	case STATE_LISTEN:
		...
		while (1) {
			rc = x_accept4(listen_fd, SOCK_NONBLOCK);
			if (rc < 0) {
				break;
			}
			fd = rc;
	...
	case STATE_SERVER:
		...
		if (revents & POLLIN) {
			rc = gtt_read_record(cp->conn_fd, g_buf, sizeof(g_buf),
					     &len);
			if (rc <= 0) {
				fin = 1;
			}
			if (len > 0) {
				rc = gtt_write_record(cp->conn_fd, g_http,
						      g_http_len);
				if (rc < 0) {
					fin = 1;
				}
	...
	case STATE_CONNECT:
		if (revents & POLLERR) {
			...
			rc = x_getsockopt(cp->conn_fd, SOL_SOCKET, SO_ERROR,
					  &errnum, &optlen);
			if (rc < 0) {
				errnum = 0;
			}
			...
			rc = gtt_write_record(cp->conn_fd, g_http, g_http_len);
		}
		if (rc < 0) {
			close(cp->conn_fd);
			worker->wrk_conns--;
...
usage(void)
{
	printf("Usage: %s [options] {-a cpus} -l [address]\n"
...
main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "hvdp:lc:n:Ca:t")) != -1) {
		...
		case 'C':
			...
			break;
		case 'a':
			rc = gtt_cpuset_from_string(&worker_cpus, optarg);
			if (rc < 0) {
				gtt_die(-rc, "-a: Invalid cpu list");
			}
			break;
		case 't':
			tflag = 1;
	...
	if (g_lflag) {
		...
		if (rc) {
			gtt_die(errno, "gethostname() failed");
		}
		g_http_len = snprintf(g_http, sizeof(g_http),
				      "GET / HTTP/1.0\r\n"
				      "Host: %s\r\n"
	...
	if (optind < argc) {
		...
		if (rc != 1) {
			gtt_die(0, "Invalid address: '%s'", argv[optind]);
		}
	} else {
		if (g_lflag == 0) {
			usage();
			gtt_die(0, "Address or '-l' flag must be specified");
		}
	}
	gtt_start_master(&worker_cpus, concurrency, PROG_NAME,
			 g_lflag ? port : 0, nflag, worker_loop,
			 tflag ? NULL : fork, sleep);
	return 0;
}
