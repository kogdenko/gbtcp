...
class Netstat:
    ...
    class Table:
        ...
        def __sub__(self, right):
            ...
            for entry in self.entries:
                value = entry.value
                if right:
                    right_entry = right.get_entry(entry.name)
                    if right_entry:
    ...
    @staticmethod
    def registered():
        return Netstat.Registered.__subclasses__()

class LinuxNetstat(Netstat, Netstat.Registered):
    @staticmethod
    def get_name():
        return "linux"
    ...
    def __init__(self):
        ...
        self.protocols = {
            ...
            'ip': [
                ...
                [r"(\d+) with bad options", "badoptions"],
                ...
                [r"(\d+) fragments dropped after timeout", "fragtimeout"],
                [r"(\d+) packets reassembled ok", "reassembled"],
            ...
            'tcp': [
                ...
                [r"(\d+) data packets \((\d+) bytes\) retransmitted", "sndrexmitpack", "sndrexmitbyte"],
                ...
                [r"(\d+) completely duplicate packets \((\d+) bytes\)", "rcvduppack", "rcvdupbyte"],
                [r"(\d+) old duplicate packets", "pawsdrop"],
                [r"(\d+) packets with some dup. data \((\d+) bytes duped\)", "rcvpartduppack", "rcvpartdupbyte"],
                [r"(\d+) out-of-order packets \((\d+) bytes\)", "rcvoopack", "rcvoobyte"],
                [r"(\d+) packets \((\d+) bytes\) of data after window", "rcvpackafterwin", "rcvbyteafterwin"],
            ...
            'udp': [
                ...
                [r"(\d+) broadcast/multicast datagrams undelivered", "noportbcast"],
            ...
            if line[-1] == ':':
                ...
                for pattern in patterns:
                    if BSDNetstat.search(table, line, pattern[0], pattern[1:]):
                        ...
                        break
                if not found:
                    log_error(None ,f"Unknown bsd statistic variable: '{table.name}:{line}'")
...
class GbtcpNetstat(BSDNetstat, Netstat.Registered):
    ...
    def read(self, app):
        cmd = singleton.builddir + "/gbtcp-netstat -nss"
        out = system(cmd)[1]
