...
#define PRELOAD_CALL(fn, ...) \
	PRELOAD_CALL_FD(GT_FIRST_FD, fn, ##__VA_ARGS__)
...
PRELOAD_SOCKET(int domain, int type, int protocol)
{
	...
	if (domain == AF_INET) {
		rc = PRELOAD_CALL(socket, domain, type, protocol);
		if (!(rc == -1 && gt_errno == ENOTSUP)) {
			return rc;
		}
	}
...
PRELOAD_BIND(int fd, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	rc = PRELOAD_CALL_FD(fd, bind, fd, addr, addrlen);
	return rc;
}
...
PRELOAD_CONNECT(int fd, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	rc = PRELOAD_CALL_FD(fd, connect, fd, addr, addrlen);
	return rc;
}
...
PRELOAD_LISTEN(int fd, int backlog)
{
	...
	rc = PRELOAD_CALL_FD(fd, listen, fd, backlog);
	return rc;
}
	...
	int flags)
{
	...
	rc = PRELOAD_CALL_FD(fd, accept4, fd, addr, addrlen, flags);
	return rc;
}
...
PRELOAD_SHUTDOWN(int fd, int how)
{
	...
	rc = PRELOAD_CALL_FD(fd, shutdown, fd, how);
	return rc;
}
...
PRELOAD_CLOSE(int fd)
{
	...
	rc = PRELOAD_CALL_FD(fd, close, fd);
	return rc;
}
...
PRELOAD_READ(int fd, void *buf, size_t count)
{
	...
	rc = PRELOAD_CALL_FD(fd, read, fd, buf, count);
	return rc;
}
...
PRELOAD_READV(int fd, const struct iovec *iov, int iovcnt)
{
	...
	rc = PRELOAD_CALL_FD(fd, readv, fd, iov, iovcnt);
	return rc;
}
...
PRELOAD_RECV(int fd, void *buf, size_t len, int flags)
{
	...
	rc = PRELOAD_CALL_FD(fd, recv, fd, buf, len, flags);
	return rc;
}
	...
	struct sockaddr *src_addr, socklen_t *addrlen)
{
	...
	rc = PRELOAD_CALL_FD(fd, recvfrom,
...
PRELOAD_RECVMSG(int fd, struct msghdr *msg, int flags)
{
	...
	rc = PRELOAD_CALL_FD(fd, recvmsg, fd, msg, flags);
	return rc;
}
...
PRELOAD_WRITE(int fd, const void *buf, size_t count)
{
	...
	rc = PRELOAD_CALL_FD(fd, write, fd, buf, count);
	return rc;
}
...
PRELOAD_WRITEV(int fd, const struct iovec *iov, int iovcnt)
{
	...
	rc = PRELOAD_CALL_FD(fd, writev, fd, iov, iovcnt);
	return rc;
}
...
PRELOAD_SEND(int fd, const void *buf, size_t len, int flags)
{
	...
	rc = PRELOAD_CALL_FD(fd, send, fd, buf, len, flags);
	return rc;
}
...
PRELOAD_SENDMSG(int fd, const struct msghdr *msg, int flags)
{
	...
	rc = PRELOAD_CALL_FD(fd, sendmsg, fd, msg, flags);
	return rc;
}
...
PRELOAD_FCNTL(int fd, int cmd, ...)
{
	...
	va_end(ap);
	rc = PRELOAD_CALL_FD(fd, fcntl, fd, cmd, arg);
	return rc;
}
...
PRELOAD_IOCTL(int fd, unsigned long request, ...)
{
	...
	arg = va_arg(ap, uintptr_t);
	va_end(ap);
	rc = PRELOAD_CALL_FD(fd, ioctl, fd, request, arg);
	return rc;
}
	...
	socklen_t *optlen)
{
	...
	rc = PRELOAD_CALL_FD(fd, getsockopt,
	...
	socklen_t optlen)
{
	...
	rc = PRELOAD_CALL_FD(fd, setsockopt,
		fd, level, optname, optval, optlen);
	return rc;
}
...
PRELOAD_GETPEERNAME(int fd, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	rc = PRELOAD_CALL_FD(fd, getpeername, fd, addr, addrlen);
	return rc;
}
...
PRELOAD_SIGPROCMASK(int how, const sigset_t *set, sigset_t *oldset)
{
	...
	rc = PRELOAD_CALL(sigprocmask, how, set, oldset);
	return rc;
}
...
PRELOAD_EPOLL_CREATE1(int flags)
{
	...
	rc = PRELOAD_CALL(epoll_create1, flags);
	return rc;	
}
...
PRELOAD_EPOLL_CTL(int ep_fd, int op, int fd, struct epoll_event *event)
{
	...
	rc = PRELOAD_CALL_FD(ep_fd, epoll_ctl, ep_fd, op, fd, event);
	return rc;
}
	...
	int timeout, const sigset_t *sigmask)
{
	...
	rc = PRELOAD_CALL_FD(ep_fd, epoll_pwait,
