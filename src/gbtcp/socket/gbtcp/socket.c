...
#define so_sid so_file.fl_worker_index
...
#define TCP_FLAG_FOREACH(x) \
	x(GT_TCPF_FIN, 'F') x(GT_TCPF_SYN, 'S') x(GT_TCPF_RST, 'R') \
		x(GT_TCPF_PSH, 'P') x(GT_TCPF_ACK, '.') x(GT_TCPF_URG, 'U')
...
#define curmod gt_so_main

// subr
static const char *tcp_flags_str(struct strbuf *sb, int proto,
				 uint8_t tcp_flags);

static const char *gt_log_add_sock(struct sock *so) __attribute__((unused));
...
static void tcp_tx_gt_timer_set(struct sock *);

static int tcp_wprobe_gt_timer_set(struct sock *);

static void tcp_gt_timer_set_tcp_fin_timeout(struct sock *);
...
static int gt_tcp_send(struct sock *so, const struct iovec *iov, int iovcnt,
		       int flags);

// udp
static int gt_udp_rcvbuf_recv(struct sock *so, const struct iovec *iov,
			      int iovcnt, struct sockaddr *addr,
			      socklen_t *addrlen, int peek);
...
static void tcp_tx_data(struct route_entry *, struct dev_pkt *, struct sock *,
			uint8_t, u_int);
...
#define GT_SOCK_ALIVE(so) ((so)->so_file.fl_freed == 0)
...
{
	return file_get_fd((struct gt_file *)so);
}
...
so_err_unpack(int so_errnum)
{
	...
	case SO_ETIMEDOUT:
		return ETIMEDOUT;
	case SO_ECONNREFUSED:
		return ECONNREFUSED;
	case SO_ECONNRESET:
		return ECONNRESET;
	case SO_EADDRINUSE:
		return EADDRINUSE;
	case SO_EADDRNOTAVAIL:
		return EADDRNOTAVAIL;
	case SO_EHOSTUNREACH:
		return EHOSTUNREACH;
	case SO_EMSGSIZE:
		return EMSGSIZE;
	default:
...
int
gt_gbtcp_so_get_err(struct gt_file *fp)
{
	int errnum;
	struct sock *so;
...
short
gt_gbtcp_so_get_events(struct gt_file *fp)
{
	short events;
	struct sock *so;
...
int
gt_gbtcp_so_nread(struct gt_file *fp)
{
	struct sock *so;
...
int
gt_gbtcp_so_socket(struct gt_file **fpp, int fd, int domain, int type,
		   int proto)
{
	struct sock *so;
...
int
gt_gbtcp_so_connect(struct gt_file *fp, const struct sockaddr_in *faddr_in)
{
	int rc;
	struct sock *so;
	...
	assert(!so_in_txq(so));
	rc = gt_so_bind_ephemeral(&so->so_base, faddr_in->sin_addr.s_addr,
				  faddr_in->sin_port);
	if (rc < 0) {
		return rc;
	}
	...
	TCP_DBG("Socket connect, %s:%hu>%s:%hu, fd:%d",
		log_add_ipaddr(AF_INET, &so->so_laddr), ntoh16(so->so_lport),
		log_add_ipaddr(AF_INET, &so->so_faddr), ntoh16(so->so_fport),
		so_get_fd(so));
...
int
gt_gbtcp_so_listen(struct gt_file *fp, int backlog)
{
	struct sock *so;
...
int
gt_gbtcp_so_accept(struct gt_file **fpp, struct gt_file *lfp)
{
	struct sock *so, *lso;
...
int
gt_gbtcp_so_close(struct gt_file *fp)
{
	struct sock *so;
...
int
gt_gbtcp_so_recvfrom(struct gt_file *fp, const struct iovec *iov, int iovcnt,
...
int
gt_gbtcp_so_aio_recvfrom(struct gt_file *fp, struct iovec *iov, int flags,
			 struct sockaddr *addr, socklen_t *addrlen)
{
	...
	if (so->so_rcvbuf.sob_len == 0) {
		if (cur_zerocopy_in == NULL || cur_zerocopy_in->in_len == 0) {
			if (so->so_rfin) {
				return 0;
			} else {
				return -EAGAIN;
...
int
gt_gbtcp_so_recvdrain(struct gt_file *fp, int len)
{
	int rc, off;
	struct sock *so;
	...
	if (off < len && cur_zerocopy_in != NULL) {
		rc = GT_MAX(len - off, cur_zerocopy_in->in_len);
...
int
gt_gbtcp_so_sendto(struct gt_file *fp, const struct iovec *iov, int iovcnt,
		   int flags, const struct sockaddr_in *dest_addr)
{
	int rc;
	struct sock *so;
...
#ifdef __linux__
int
gt_gbtcp_so_ioctl(struct gt_file *fp, unsigned long request, uintptr_t arg)
{
	return -ENOTSUP;
}
#else /* __linux */
...
int
gt_gbtcp_so_getsockopt(struct gt_file *fp, int level, int optname, void *optval,
		       socklen_t *optlen)
{
...
int
gt_gbtcp_so_setsockopt(struct gt_file *fp, int level, int optname,
...
int
gt_gbtcp_so_getpeername(struct gt_file *fp, struct sockaddr *addr,
			socklen_t *addrlen)
{
	int rc;
	struct sock *so;
...
tcp_set_rmss(struct sock *so, struct tcp_opts *opt)
{
	if (opt->tcp_opt_flags & (1 << TCP_OPT_MSS)) {
		so->so_rmss = GT_MAX(IP4_MTU_MIN - 20, opt->tcp_opt_mss);
	} else {
		so->so_rmss = 536;
	}
}
...
tcp_set_state(struct sock *so, struct in_context *in, int state)
{
	...
	TCP_DBG("Socket state transition %s->%s, fd=%d",
		tcp_state_str(so->so_state), tcp_state_str(state),
		so_get_fd(so));
...
{
	so->so_ssnt = 0;
	gt_timer_cancel(&so->so_timer);
	gt_timer_cancel(&so->so_timer_delack);
...
tcp_delack(struct sock *so)
{
	...
	gt_timer_set_fn(&so->so_timer_delack, 200 * GT_NSEC_PER_MSEC,
			gt_so_main->so_timer_delack_fn);
}

static void
tcp_tx_gt_timer_set(struct sock *so)
{
	uint64_t expires;
	...
	if (!tcps_is_connected(so->so_state)) {
		expires = GT_NSEC_PER_SEC;
	} else {
		expires = 500 * GT_NSEC_PER_MSEC;
	}
	expires <<= so->so_ntries;
	gt_timer_set_fn(&so->so_timer, expires, gt_so_main->so_timer_rexmit_fn);
}

static int
tcp_wprobe_gt_timer_set(struct sock *so)
{
	uint64_t expires;
	...
	expires = 10 * GT_NSEC_PER_SEC;
	gt_timer_set_fn(&so->so_timer, expires,
			gt_so_main->so_timer_persist_fn);
	return 1;
}

static void
tcp_gt_timer_set_tcp_fin_timeout(struct sock *so)
{
	assert(so->so_retx == 0);
	assert(so->so_wprobe == 0);
	assert(!gt_timer_is_running(&so->so_timer));
	gt_timer_set_fn(&so->so_timer, curmod->tcp_fin_timeout,
			gt_so_main->so_timer_fin_fn);
}

void
gt_gbtcp_tcp_timer_delack(struct gt_timer *timer)
{
	struct sock *so;
...
void
gt_gbtcp_tcp_timer_rexmit(struct gt_timer *timer)
{
	struct sock *so;
	...
	TCP_DBG("Retransmit timeout, fd=%d, state=%s", so_get_fd(so),
...
void
gt_gbtcp_tcp_timer_persist(struct gt_timer *timer)
{
	struct sock *so;
	...
	tcp_into_ackq(so);
	tcp_wprobe_gt_timer_set(so);
}

void
gt_gbtcp_tcp_timer_fin(struct gt_timer *timer)
{
	struct sock *so;
...
void
gt_gbtcp_tcp_timer_time_wait(struct gt_timer *timer)
{
	struct sock *so;
...
static void
tcp_rcv_LISTEN(struct sock *lso, struct in_context *in, be32_t laddr,
	       be32_t faddr, be16_t lport, be16_t fport)
{
	...
	if (in->in_tcp_flags != GT_TCPF_SYN) {
		TCP_DBG("First connection packet not a SYN [%s]; %s:%hu>%s:%hu, seq=%u, lfd=%d, fd=%d",
			...
			ntoh16(so->so_fport), in->in_tcp_seq, so_get_fd(lso),
			so_get_fd(so));
		//abort();
		tcpstat.tcps_badsyn++;
		tcp_reset(so, in);
		...
		TCP_DBG("New incoming connection %s:%hu>%s:%hu, lfd=%d, fd=%d",
			...
			ntoh16(so->so_lport),
			log_add_ipaddr(AF_INET, &so->so_faddr),
			ntoh16(so->so_fport), so_get_fd(lso), so_get_fd(so));
	}
...
tcp_rcv_established(struct sock *so, struct in_context *in)
{
	...
	if (in->in_tcp_flags & GT_TCPF_FIN) {
		...
		case GT_TCPS_FIN_WAIT_1:
			...
			break;
		case GT_TCPS_FIN_WAIT_2:
			gt_timer_cancel(&so->so_timer); // tcp_fin_timeout
...
tcp_enter_time_wait(struct sock *so, struct in_context *in)
{
	...
	if (to == 0) {
		...
		gt_timer_set_fn(&so->so_timer, to,
				gt_so_main->so_timer_time_wait_fn);
		return 0;
	}
}
...
tcp_process_ack_complete(struct sock *so, struct in_context *in)
{
	...
	if (so->so_sfin && so->so_sfin_acked == 0) {
		...
		switch (so->so_state) {
		case GT_TCPS_FIN_WAIT_1:
			tcp_gt_timer_set_tcp_fin_timeout(so);
	 ...
	 uint8_t tcp_flags, u_int len)
{
	...
	if (tcb->tcb_len) {
		...
		sockbuf_copy(&so->so_sndbuf, so->so_ssnt, payload,
			     tcb->tcb_len);
	}
...
gt_gbtcp_so_tx_flush(void)
{
	...
	struct gt_socket_worker *worker;

	worker = &gt_so_main->so_workers[gt_get_worker_index()];

	tx_head = &worker->sow_tx_pending_head;
	while (!gt_dlist_is_empty(tx_head)) {
		so = GT_DLIST_FIRST(tx_head, struct sock, so_tx_list);
		rc = so_route(&so->so_base, &r);
		assert(rc == 0);
		do {
			rc = route_get_tx_packet(r.rt_ifp, &pkt,
						 TX_CAN_REDIRECT);
			if (rc) {
				return -EAGAIN;
			}
		   ...
		   struct sockaddr *addr, socklen_t *addrlen, int peek)
{
	...
	cnt = sockbuf_readv(&so->so_rcvbuf, iov, iovcnt, msg.sobm_len, peek);
	TCP_DBG("Receive from UDP socket, peek=%d, cnt=%d, buflen=%d, fd=%d",
		peek, rc, so->so_rcvbuf.sob_len, so_get_fd(so));
	if (peek == 0) {
		if (msg.sobm_len > cnt) {
			msg.sobm_len -= cnt;
			rc = sockbuf_rewrite(&so->so_msgbuf, &msg, sizeof(msg));
			assert(rc == sizeof(msg));
		} else {
...
sock_str(struct strbuf *sb, struct sock *so)
{
	...
	strbuf_addf(sb, "{ proto=%s, fd=%d, tuple=", is_tcp ? "tcp" : "udp",
		    so_get_fd(so));
	...
	strbuf_addf(sb,
		    ...
		    ", lmss=%u"
		    ", rmss=%u",
		    ntoh16(so->so_fport), so_in_txq(so), so->so_err,
		    so->so_reuseaddr, so->so_reuseport,
		    gt_timer_is_running(&so->so_timer), so->so_lmss,
		    so->so_rmss);
	if (is_tcp) {
		if (so->so_is_listen) {
			strbuf_addf(sb,
				    ", backlog=%u"
				    ", acceptq_len=%u",
				    so->so_backlog, so->so_acceptq_len);
		} else {
			strbuf_addf(sb,
				    ", state=%s"
				    ", passive_open=%u"
				    ...
				    ", sndbuf_len=%u",
				    tcp_state_str(so->so_state),
				    so->so_passive_open, so->so_accepted,
				    so->so_ack, so->so_wprobe, so->so_retx,
				    so->so_ntries, so->so_dont_frag,
				    so->so_rshut, so->so_rsyn, so->so_rfin,
				    so->so_ssyn, so->so_ssyn_acked, so->so_sfin,
				    so->so_sfin_sent, so->so_sfin_acked,
				    so->so_nagle, so->so_nagle_acked,
				    gt_timer_is_running(&so->so_timer_delack),
				    so->so_rseq, so->so_sack, so->so_ssnt,
				    so->so_swnd, so->so_rwnd, so->so_rwnd_max,
				    so->so_ip_id, so->so_rcvbuf.sob_len,
		...
		strbuf_addf(sb,
			    ", rcvbuf_len=%u"
			    ", msgbuf_len=%u",
			    so->so_rcvbuf.sob_len, so->so_msgbuf.sob_len);
	}
	strbuf_add_ch(sb, '}');
...
so_add_txq(struct route_if *ifp, struct sock *so)
{
	...
	GT_DLIST_INSERT_TAIL(&w->sow_tx_pending_head, so, so_tx_list);
}
...
static int
so_rcvbuf_add(struct sock *so, void *buf,
	      int len /*, be32_t faddr, be16_t fport*/)
{
	int rc;
	...
	/*		assert(rc >= 0);
		...
		if (so->so_base.sobase_proto == IPPROTO_UDP && rc > 0) {
			...
			msg.sobm_len = rc;
			rc = sockbuf_add(&current->sow_sockbuf_pool,
	    ...
	    uint8_t tcp_flags, u_int len)
{
	...
	TCP_DBG("Transmit packet [%s] via %s, len=%d, seq=%u, ack=%u, fd=%d",
		...
		r->rt_ifp->rif_name, tcb.tcb_len, tcb.tcb_seq, tcb.tcb_ack,
		so_get_fd(so));
...
sock_sndbuf_add(struct sock *so, const void *src, int cnt)
{
	...
	TCP_DBG("Add %d bytes to send-buffer, buflen=%d, fd=%d", cnt,
		so->so_sndbuf.sob_len, so_get_fd(so));
	return rc;
}
...
sock_sndbuf_drain(struct sock *so, int cnt)
{
	...
	TCP_DBG("Remove %d bytes from send-buffer, buflen=%d, fd=%d", cnt,
		so->so_sndbuf.sob_len, so_get_fd(so));
}
	 ...
	 be16_t lport, be16_t fport)
{
	...
	rc = gt_so_lookup((struct gt_sock **)&so, proto, laddr, faddr, lport,
			  fport);
	if (rc != IN_OK) {
		return rc;
	}
	...
	TCP_DBG("Receive packet [%s], len=%d, seq=%u, ack=%u, fd=%d",
		log_add_tcp_flags(proto, in->in_tcp_flags), in->in_len,
		in->in_tcp_seq, in->in_tcp_ack, so_get_fd(so));
	so->so_processing = 1;
	in->in_events = 0;
	if (in->in_len) {
		if (so->so_rshut) {
...
gt_gbtcp_so_rx(struct route_if *ifp, void *data, int len)
{
	...
	if (proto == IPPROTO_UDP || proto == IPPROTO_TCP) {
		rc = so_input(proto, p, p->in_ih->ih_daddr, p->in_ih->ih_saddr,
		...
		rc = so_input_err(p->in_emb_ipproto, p, p->in_ih->ih_daddr,
				  p->in_ih->ih_saddr, p->in_uh->uh_dport,
				  p->in_uh->uh_sport);
	} else {
		rc = IN_BYPASS;
	}
