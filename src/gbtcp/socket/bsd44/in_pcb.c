...
int
in_pcbconnect(struct socket *so, const struct sockaddr_in *faddr_in,
	      uint32_t *ph)
{
	int rc;
...
int
in_pcblookup(struct socket **pso, int proto, be32_t laddr, be32_t faddr,
	     be16_t lport, be16_t fport)
{
	int rc;

	rc = gt_so_lookup((struct gt_sock **)pso, proto, laddr, faddr, lport,
			  fport);
