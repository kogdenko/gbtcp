...
char *prurequests[] = {
	[PRU_DETACH] = "DETACH",     [PRU_BIND] = "BIND",
	[PRU_LISTEN] = "LISTEN",     [PRU_CONNECT] = "CONNECT",
	[PRU_ACCEPT] = "ACCEPT",     [PRU_DISCONNECT] = "DISCONNECT",
	[PRU_SHUTDOWN] = "SHUTDOWN", [PRU_SEND] = "SEND",
	[PRU_ABORT] = "ABORT",	     [PRU_FASTTIMO] = "FASTTIMO",
	[PRU_SLOWTIMO] = "SLOWTIMO",
};

static const char *tanames[] = { [TA_INPUT] = "input",
				 [TA_OUTPUT] = "output",
				 [TA_USER] = "user",
				 [TA_RESPOND] = "respond",
				 [TA_DROP] = "drop" };

static const char *tcptimers[] = { [TCPT_REXMT] = "REXMT",
 ...
 */
void
tcp_trace(int act, int ostate, struct tcpcb *tp, struct ip4_hdr *ip,
	  struct tcp_hdr *th, int req)
{
	...
	if (tp) {
		so = tcpcbtoso(tp);
		printf("[%s:%hu > %s:%hu %s] ",
		       inet_ntop(AF_INET, &so->so_base.sobase_laddr, lb,
				 sizeof(lb)),
		       ntohs(so->so_base.sobase_lport),
		       inet_ntop(AF_INET, &so->so_base.sobase_faddr, fb,
				 sizeof(fb)),
		       ntohs(so->so_base.sobase_fport), tcpstates[ostate]);
	} else {
		printf("[?] ");
	}
	switch (act) {
