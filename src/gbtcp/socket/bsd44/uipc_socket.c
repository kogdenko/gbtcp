...
#include <gbtcp/socket/bsd44/udp_var.h>
 ...
 */
int
gt_bsd44_so_socket(struct gt_file **fpp, int fd, int domain, int type,
		   int proto)
{
	struct socket *so;
...
int
gt_bsd44_so_listen(struct gt_file *fp, int backlog)
{
	int error;
	struct socket *so;
...
sofree(struct socket *so)
{
	...
	if (so->so_options & SO_OPTION(SO_ACCEPTCONN)) {
		for (q = 0; q < GT_ARRAY_SIZE(so->so_q); ++q) {
 ...
 */
int
gt_bsd44_so_close(struct gt_file *fp)
{
	int q, rc;
...
int
gt_bsd44_so_connect(struct gt_file *fp, const struct sockaddr_in *faddr_in)
{
	int rc;
	struct socket *so;
...
int
gt_bsd44_so_setsockopt(struct gt_file *fp, int level, int optname,
...
int
gt_bsd44_so_getsockopt(struct gt_file *fp, int level, int optname, void *optval,
		       socklen_t *optlen)
{
	int rc;
...
#define sbchspace(ch) (SBCHUNK_DATASIZE - ((ch)->sbc_len + (ch)->sbc_off))

static void
sbchfree(struct sockbuf_chunk *ch)
{
	gt_free_internal(shm_cache(), ch);
}
...
sbchalloc(struct sockbuf *sb)
{
	...
	ch = gt_malloc(shm_cache(), SOCKBUF_CHUNK_SIZE, 0);
	if (ch == NULL) {
		return NULL;
	}
	ch->sbc_len = 0;
...
sbfree_n(struct sockbuf *sb, int n)
{
	...
	for (i = 0; i < n; ++i) {
		...
		ch = GT_DLIST_LAST(&sb->sb_head, struct sockbuf_chunk,
				   sbc_list);
...
sbrelease(struct sockbuf *sb)
{
	...
	while (!gt_dlist_is_empty(&sb->sb_head)) {
		ch = GT_DLIST_FIRST(&sb->sb_head, struct sockbuf_chunk,
				    sbc_list);
		GT_DLIST_REMOVE(ch, sbc_list);
		sbchfree(ch);
	}
...
int
gt_bsd44_so_accept(struct gt_file **fpp, struct gt_file *lfp)
{
	int error;
...
int
gt_bsd44_so_sendto(struct gt_file *fp, const struct iovec *iov, int iovcnt,
...
int
gt_bsd44_so_get_err(struct gt_file *fp)
{
	struct socket *so;
...
int
gt_bsd44_so_getpeername(struct gt_file *fp, struct sockaddr *addr,
			socklen_t *addrlen)
{
	int rc;
	struct socket *so;
	...
	rc = gt_set_sockaddr(addr, addrlen, so->so_base.sobase_faddr,
			     so->so_base.sobase_fport);
...
gt_bsd44_so_tx_flush(void)
{
	...
	tcp_now = 1 + shared_ns() / (GT_NSEC_PER_SEC / PR_SLOWHZ);

	tx_head = &gt_so_main->so_workers[gt_get_worker_index()]
			   .sow_tx_pending_head;
	while (!gt_dlist_is_empty(tx_head)) {
		so = GT_DLIST_FIRST(tx_head, struct socket, so_txlist);
		rc = gt_so_route(so->so_base.sobase_laddr,
				 so->so_base.sobase_faddr, &r);
		assert(rc == 0);
		for (;;) {
...
short
gt_bsd44_so_get_events(struct gt_file *fp)
{
	struct socket *so;
...
int
gt_bsd44_so_nread(struct gt_file *fp)
{
	struct socket *so;
...
int
gt_bsd44_so_recvfrom(struct gt_file *fp, const struct iovec *iov, int iovcnt,
		     int flags, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	for (i = 0; i < iovcnt; ++i) {
		...
		len = GT_MIN(sb->sb_cc, iov[i].iov_len);
...
int
gt_bsd44_so_aio_recvfrom(struct gt_file *fp, struct iovec *iov, int flags,
			 struct sockaddr *addr, socklen_t *addrlen)
{
	...
	iov->iov_base = sbchdata(ch) + ch->sbc_off;
	iov->iov_len = ch->sbc_len;
	return ch->sbc_len;
}

int
gt_bsd44_so_recvdrain(struct gt_file *fp, int len)
{
	int cc;
	struct socket *so;
...
int
gt_bsd44_so_ioctl(struct gt_file *fp, unsigned long request, uintptr_t arg)
{
	return -ENOTSUP;
}
