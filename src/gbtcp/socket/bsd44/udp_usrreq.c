...
#include <gbtcp/socket/bsd44/ip_icmp.h>
...
udp_input(struct ip4_hdr *ip, int iphlen, int eth_flags)
{
	...
	rc = in_pcblookup(&so, IPPROTO_UDP, ip->ih_daddr, ip->ih_saddr,
...
udp_ctlinput(int err, be32_t dst, struct ip *ip)
{
	...
	in_pcbnotify(IPPROTO_UDP, ip->ip_src.s_addr, uh->uh_sport, dst,
		     uh->uh_dport, err, udp_notify);
}
...
int
udp_send(struct socket *so, const void *dat, int datlen,
	 const struct sockaddr_in *addr)
{
