...
tcp_dooptions(struct route_if *ifp, struct tcpcb *tp, u_char *cp, int cnt,
	      struct tcp_hdr *th, int *ts_present, uint32_t *ts_val,
	      uint32_t *ts_ecr)
{
	u_short mss;
	int opt, optlen;
	...
	for (; cnt > 0; cnt -= optlen, cp += optlen) {
		...
		case TCPOPT_WINDOW:
			...
			tp->requested_s_scale = GT_MIN(cp[2], TCP_MAX_WINSHIFT);
			break;
...
tcp_xmit_timer(struct tcpcb *tp, short rtt)
{
	 ...
	 */
	TCPT_RANGESET(tp->t_rxtcur, TCP_REXMTVAL(tp), TCPTV_MIN,
		      TCPTV_REXMTMAX);
...
findpcb:
	again = 0;
	datlen = 0;
	rc = in_pcblookup(&so, IPPROTO_TCP, ip->ih_daddr, ip->ih_saddr,
	...
	if (optp != NULL && tp->t_state != GT_TCPS_LISTEN) {
		tcp_dooptions(ifp, tp, optp, optlen, th, &ts_present, &ts_val,
			      &ts_ecr);
	}
	 ...
	 */
	rcv_wnd = GT_MAX(so->so_rcv.sb_hiwat, (int)(tp->rcv_adv - tp->rcv_nxt));
	...
	case GT_TCPS_LISTEN:
		...
		if (optp != NULL) {
			tcp_dooptions(ifp, tp, optp, optlen, th, &ts_present,
				      &ts_val, &ts_ecr);
		}
	...
	case GT_TCPS_SYN_SENT:
		...
		gt_timer_cancel(tp->t_timer + TCPT_REXMT);
		tcp_rcvseqinit(tp, th->th_seq);
		tp->t_flags |= TF_ACKNOW;
		if ((flags & GT_TCPF_ACK)) {
	...
	case GT_TCPS_TIME_WAIT:
		if (SEQ_LEQ(th->th_ack, tp->snd_una)) {
			if (ip->ih_total_len == 0 &&
				 ...
				 * network.
				 */
				if (!gt_timer_is_running(tp->t_timer +
							 TCPT_REXMT) ||
				    th->th_ack != tp->snd_una) {
					tp->t_dupacks = 0;
				} else if (++tp->t_dupacks == tcprexmtthresh) {
					win = GT_MIN(tp->snd_wnd, tp->snd_cwnd);
					win = win / 2 / tp->t_maxseg;
					if (win < 2) {
						win = 2;
					}
					tp->snd_ssthresh = win * tp->t_maxseg;
					gt_timer_cancel(tp->t_timer +
							TCPT_REXMT);
					tp->t_rtt = 0;
		...
		if (th->th_ack == tp->snd_max) {
			gt_timer_cancel(tp->t_timer + TCPT_REXMT);
			needoutput = 1;
		} else if (!gt_timer_is_running(tp->t_timer + TCPT_PERSIST))
		 ...
		 */
		{
			...
			tp->snd_cwnd =
				GT_MIN(cw + incr, TCP_MAXWIN << tp->snd_scale);
		}
...
step6:
	 ...
	 */
	if ((flags & GT_TCPF_ACK) && SEQ_LEQ(tp->snd_wl1, th->th_seq) &&
	    SEQ_LEQ(tp->snd_wl2, th->th_ack) && tiwin > tp->snd_wnd) {
	...
	if ((ip->ih_total_len || (flags & GT_TCPF_FIN)) &&
		...
		if (th->th_seq == tp->rcv_nxt) {
			if (tp->t_flags & TF_DELACK) {
				...
				gt_timer_cancel(tp->t_timer + TCPT_DELACK);
				tp->t_flags |= TF_ACKNOW;
			}
...
tcp_mss(struct route_if *ifp, struct tcpcb *tp, u_int offer)
{
	 ...
	 */
	if (offer)
		mss = GT_MIN(mss, offer);
	mss = GT_MAX(mss, 32); /* sanity */
	if (mss < tp->t_maxseg || offer != 0) {
		...
		if (bufsize < mss)
			...
			bufsize = GT_ROUND_UP(bufsize, mss);
			if (bufsize > SB_MAX)
				bufsize = SB_MAX;
