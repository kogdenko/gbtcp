...
struct tcpcb {
	...
	struct gt_timer t_timer[TCPT_NTIMERS]; /* tcp timers */
...
int tcp_output_real(struct route_entry *r, struct dev_pkt *pkt,
		    struct socket *so);
void tcp_quench(struct socket *, int);
void tcp_respond(struct socket *, struct ip4_hdr *, struct tcp_hdr *, tcp_seq,
		 tcp_seq, int);
void tcp_setpersist(struct tcpcb *);
void tcp_trace(int, int, struct tcpcb *, struct ip4_hdr *, struct tcp_hdr *,
	       int);
...
void tcp_2MSL_timo(struct gt_timer *);
void tcp_REXMT_timo(struct gt_timer *);
void tcp_PERSIST_timo(struct gt_timer *);
void tcp_KEEP_timo(struct gt_timer *);
void tcp_DELACK_timo(struct gt_timer *);

#define TCPS_HAVERCVDSYN(s) \
	((s) == GT_TCPS_SYN_RCVD || (s) == GT_TCPS_ESTABLISHED || \
	 (s) == GT_TCPS_CLOSE_WAIT || (s) == GT_TCPS_FIN_WAIT_1 || \
	 (s) == GT_TCPS_CLOSING || (s) == GT_TCPS_LAST_ACK || \
	 (s) == GT_TCPS_FIN_WAIT_2 || (s) == GT_TCPS_TIME_WAIT)
