 ...
 */
void
tcp_DELACK_timo(struct gt_timer *timer)
{
	struct tcpcb *tp;
...
{
	tp->t_flags |= TF_DELACK;
	gt_timer_set_fn(&tp->t_timer[TCPT_DELACK], 200 * GT_NSEC_PER_MSEC,
			gt_so_main->so_bsd44_timer_fn[TCPT_DELACK]);
}
 ...
 */
void
tcp_PERSIST_timo(struct gt_timer *timer)
{
	struct tcpcb *tp;
 ...
 */
void
tcp_REXMT_timo(struct gt_timer *timer)
{
	u_int win, rexmt;
	struct tcpcb *tp;
	 ...
	 * to go below this.)
	 */
	win = GT_MIN(tp->snd_wnd, tp->snd_cwnd) / 2 / tp->t_maxseg;
	if (win < 2) {
		win = 2;
	}
 ...
 */
void
tcp_KEEP_timo(struct gt_timer *timer)
{
	struct socket *so;
	struct tcpcb *tp;
	...
	if (so->so_options & SO_OPTION(SO_KEEPALIVE) &&
		...
		tcp_respond(so, NULL, NULL, tp->rcv_nxt, tp->snd_una - 1, 0);
 ...
 */
void
tcp_2MSL_timo(struct gt_timer *timer)
{
	struct tcpcb *tp;
