...
tcp_output(struct tcpcb *tp)
{
	...
	GT_DLIST_INSERT_TAIL(&worker->sow_tx_pending_head, so, so_txlist);
}
...
tcp_output_real(struct route_entry *r, struct dev_pkt *pkt, struct socket *so)
{
	...
	if (t_force) {
		if (win == 0) {
			...
			gt_timer_cancel(tp->t_timer + TCPT_PERSIST);
			tp->t_rxtshift = 0;
		}
	}

	len = GT_MIN(so->so_snd.sb_cc, win) - off;
	...
	if (win > 0) {
		 ...
		 */
		long adv = GT_MIN(win, (long)TCP_MAXWIN << tp->rcv_scale) -
	 ...
	 */
	if (so->so_snd.sb_cc &&
	    !gt_timer_is_running(tp->t_timer + TCPT_REXMT) &&
	    !gt_timer_is_running(tp->t_timer + TCPT_PERSIST)) {
		tp->t_rxtshift = 0;
		tcp_setpersist(tp);
	}
...
send:
	 ...
	 */
	if (t_force == 0 || !gt_timer_is_running(tp->t_timer + TCPT_PERSIST)) {
		 ...
		 * of retransmit time.
		 */
		if (!gt_timer_is_running(tp->t_timer + TCPT_REXMT) &&
		    tp->snd_nxt != tp->snd_una) {
			tcp_setslowtimer(tp, TCPT_REXMT, tp->t_rxtcur);
			if (gt_timer_is_running(tp->t_timer + TCPT_PERSIST)) {
				gt_timer_cancel(tp->t_timer + TCPT_PERSIST);
				tp->t_rxtshift = 0;
			}
		}
...
tcp_setpersist(struct tcpcb *tp)
{
	...
	TCPT_RANGESET(timo, t * tcp_backoff[tp->t_rxtshift], TCPTV_PERSMIN,
		      TCPTV_PERSMAX);
