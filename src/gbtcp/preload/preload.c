...
// Can be called before constructor, so we must resolve symbol via `dlsym` here
#define SYS_CALL(fn, ...) \
	({ \
		GT_SYS_DLSYM_NEXT(fn); \
...
#define PRELOAD_CALL(fn, fd, flags, ...) \
	({ \
		...
		ssize_t rc; \
\
		if (GT_FLAG_ISSET(flags, PRELOAD_FLAG_PASSTHRU) || \
		    preload_called || \
		    (GT_FLAG_ISSET(flags, PRELOAD_FLAG_FD_ARG) && \
		     fd < GT_FIRST_FD)) { \
...
gt_preload_constructor(void)
{
	GT_SYS_X(GT_SYS_DLSYM_NEXT);
}
...
gt_preload_accept4(int fd, struct sockaddr *addr, socklen_t *addrlen, int flags)
{
	...
	rc = PRELOAD_CALL(accept4, fd,
			  PRELOAD_FLAG_FD_ARG | PRELOAD_FLAG_FD_RET, fd, addr,
			  addrlen, flags);
	return rc;
}
		    ...
		    struct sockaddr *src_addr, socklen_t *addrlen)
{
	...
	rc = PRELOAD_CALL(recvfrom, fd, PRELOAD_FLAG_FD_ARG, fd, buf, len,
		  ...
		  const struct sockaddr *dest_addr, socklen_t addrlen)
{
	...
	rc = PRELOAD_CALL(sendto, fd, PRELOAD_FLAG_FD_ARG, fd, buf, len, flags,
			  dest_addr, addrlen);
	return rc;
}
		      ...
		      socklen_t *optlen)
{
	...
	rc = PRELOAD_CALL(getsockopt, fd, PRELOAD_FLAG_FD_ARG, fd, level,
		      ...
		      socklen_t optlen)
{
	...
	rc = PRELOAD_CALL(setsockopt, fd, PRELOAD_FLAG_FD_ARG, fd, level,
			  optname, optval, optlen);
	return rc;
}
...
gt_preload_getpeername(int fd, struct sockaddr *addr, socklen_t *addrlen)
{
	...
	rc = PRELOAD_CALL(getpeername, fd, PRELOAD_FLAG_FD_ARG, fd, addr,
			  addrlen);
	return rc;
}
		 ...
		 ...)
{
	...
	ctid = va_arg(ap, void *);
	va_end(ap);
	rc = PRELOAD_CALL(clone, 0, 0, fn, child_stack, flags, arg, ptid, tls,
			  ctid);
	return rc;
}
...
gt_preload_epoll_ctl(int ep_fd, int op, int fd, struct epoll_event *event)
{
	...
	rc = PRELOAD_CALL(epoll_ctl, ep_fd, PRELOAD_FLAG_FD_ARG, ep_fd, op, fd,
			  event);
	return rc;
}

int
gt_preload_epoll_pwait(int ep_fd, struct epoll_event *events, int maxevents,
		       int timeout, const sigset_t *sigmask)
{
	int rc;

	rc = PRELOAD_CALL(epoll_pwait, ep_fd, PRELOAD_FLAG_FD_ARG, ep_fd,
			  events, maxevents, timeout, sigmask);
	return rc;
}

int
gt_preload_epoll_wait(int ep_fd, struct epoll_event *events, int maxevents,
		      int timeout)
{
	int rc;
		  ...
		  const struct timespec *timeout)
{
	...
	rc = PRELOAD_CALL(kevent, kq, PRELOAD_FLAG_FD_ARG, kq, changelist,
			  nchanges, eventlist, nevents, timeout);
	return rc;
}
#endif // __linux__
