...
#include <gbtcp/nl/nl.h> // FIXME: ambigious include -- should contain gbtcp
...
handle_link(struct route_msg *msg, int ifindex)
{
	...
	struct route_msg_link *m;

	msg->rtm_type = ROUTE_MSG_LINK;
	msg->rtm_ifindex = ifindex;
	rc = sys_if_indextoname(ifindex, ifname);
	if (rc) {
		return rc;
	}

	m = &msg->rtm_link;
	rc = gt_get_ifhwaddr(&m->rtml_hwaddr, &m->rtml_flags, ifname);
	if (rc) {
		return rc;
	}
...
static int
handle_rtmsg(struct rt_msghdr *rtm, size_t msg_len, route_msg_f fn, void *udata)
{
	int rc;
...
static int
route_dump_ifaddrs(route_msg_f fn, void *udata)
{
	int rc, ifindex;
	...
	for (ifa = ifap; ifa != NULL; ifa = ifa->ifa_next) {
		...
		case AF_LINK:
			...
			msg.rtm_type = ROUTE_MSG_LINK;
			msg.rtm_ifindex = ifindex;
			memcpy(&msg.rtm_link.rtml_hwaddr.eth_u8, LLADDR(dl), 6);
			msg.rtm_link.rtml_flags = ifa->ifa_flags;
			(*fn)(&msg, udata);
			break;
...
route_dump(route_msg_f fn, void *udata)
{
	...
	rc = route_dump_ifaddrs(fn, udata);
	if (rc) {
		return rc;
	}
	...
	rc = sysctlbyname("net.my_fibnum", &net_my_fibnum, &net_my_fib_num_size,
			  NULL, 0);
	if (rc == -1) {
		net_my_fibnum = 0;
	}
...
route_read(int fd, route_msg_f fn, void *udata)
{
	...
	handle_rtmsg((struct rt_msghdr *)msg, rc, fn, udata);
	return 0;
}
