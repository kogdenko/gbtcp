...
static struct log_scope *curmod;

void
gt_xdp_set_curmod(struct log_scope *m)
{
	curmod = m;
}
...
#define XDP_FRAME_NUM \
	(2 * (XSK_RING_CONS__DEFAULT_NUM_DESCS + \
...
xdp_queue_init(struct dev *dev)
{
	...
	//	if (rc < 0) {
	//		gt_err3(curmod, -rc, "bpf_xdp_query_id('%d') failed", dev->dev_ifindex);
	//		goto err;
	//	}
	...
	rc = sys_posix_memalign(&q->xq_buf, GT_SYS_PAGE_SIZE, size);
	if (rc < 0) {
		goto err;
	}
	...
	size = XDP_FRAME_NUM * XDP_FRAME_SIZE;
	rc = xsk_umem__create(&q->xq_umem, q->xq_buf, size, &q->xq_fill,
			      &q->xq_comp, NULL);
	if (rc < 0) {
		gt_err3(curmod, -rc, "xsk_umem__create() failed");
		goto err;
	}
	...
	rc = xsk_socket__create(&q->xq_xsk, dev->dev_ifname, dev->dev_queue_id,
				q->xq_umem, &q->xq_rx, &q->xq_tx, &cfg);
	if (rc < 0) {
		gt_err3(curmod, -rc, "xsk_socket__create('%s', '%d') failed",
			dev->dev_ifname, dev->dev_queue_id);
		goto err;
	}
	idx = UINT32_MAX;
	rc = xsk_ring_prod__reserve(&q->xq_fill,
				    XSK_RING_PROD__DEFAULT_NUM_DESCS, &idx);
	if (rc != XSK_RING_PROD__DEFAULT_NUM_DESCS) {
		gt_err3(curmod, 0, "xsk_ring_prod__reserve() failed");
		rc = -EINVAL;
		goto err;
	}
	assert(idx != UINT32_MAX);
	for (i = 0; i < XSK_RING_PROD__DEFAULT_NUM_DESCS; i++, idx++) {
		*xsk_ring_prod__fill_addr(&q->xq_fill, idx) =
			xdp_queue_alloc_frame(q);
	}
...
xdp_dev_init(struct dev *dev)
{
	...
	gt_notice3(curmod, 0, "Create XDP device '%s'; queue=%d",
		   dev->dev_ifname, dev->dev_queue_id);
	...
	if (dev->dev_xdp_queue == NULL) {
		gt_err3(curmod, 0, "No memory to allocate XDP queue");
		rc = -ENOMEM;
		goto err;
	}
	rc = xdp_queue_init(dev);
	if (rc >= 0) {
		gt_notice3(curmod, 0, "XDP device '%s' created; queue=%d",
			   dev->dev_ifname, dev->dev_queue_id);
		return rc;
	}
err:
	gt_err3(curmod, -rc, "Failed to create XDP device '%s'; queue=%d",
		dev->dev_ifname, dev->dev_queue_id);
...
{
	gt_notice3(curmod, 0, "Destroy XDP device '%s', queue=%d'",
		   dev->dev_ifname, dev->dev_queue_id);
...
xdp_dev_rx(struct dev *dev)
{
	...
	if (m > 0) {
		...
		for (i = 0; i < m; ++i, ++idx_fill) {
			...
			*xsk_ring_prod__fill_addr(&q->xq_fill, idx_fill) =
				frame;
		}
...
xdp_dev_tx_flush(struct dev *dev)
{
	...
	idx = UINT32_MAX;
	n = xsk_ring_cons__peek(&q->xq_comp, XSK_RING_CONS__DEFAULT_NUM_DESCS,
				&idx);
	if (n <= 0) {
		return;
	}
