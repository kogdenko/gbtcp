...
struct tcp_opt_info tcp_opt_info[TCP_OPT_MAX] = { { TCP_OPT_MSS, 4 },
						  { TCP_OPT_WSCALE, 3 },
						  { TCP_OPT_SACK_PERMITED, 2 },
						  { TCP_OPT_TIMESTAMPS, 10 } };

#define IP4_L4_LEN(ih) \
	(ntoh16((ih)->ih_total_len) - IP4_HDR_LEN((ih)->ih_ver_ihl))
...
struct gt_dlist gt_inet_stat_head;

static void
gt_inet_add_stat(const char *proto, const char *name, uintptr_t off)
{
	struct gt_inet_stat *stat;

	stat = sys_malloc(sizeof(*stat));
	stat->proto_name = sys_strdup(proto);
	stat->stat_name = sys_strdup(name);
	stat->off = off;

	GT_DLIST_INSERT_HEAD(&gt_inet_stat_head, stat, link);
}

static void
gt_inet_add_tcp_stats(void)
{
	int i, off;
	char name[64];

#define SYSCTL_ADD_TCP_STAT(x) \
	gt_inet_add_stat("tcp", #x, \
			 field_off(struct service, p_tcpstat.tcps_##x));
...
#undef SYSCTL_ADD_TCP_STAT
	for (i = 0; i < GT_TCPS_MAX_STATES; ++i) {
		...
		gt_inet_add_stat("tcp", name, off);
	}
}

static void
gt_inet_add_udp_stats(void)
{
#define SYSCTL_ADD_UDP_STAT(x) \
	gt_inet_add_stat("udp", #x, \
			 field_off(struct service, p_udpstat.udps_##x));
	GT_X_UDP_STAT(SYSCTL_ADD_UDP_STAT)
#undef SYSCTL_ADD_UDP_STAT
}

static void
gt_inet_add_ip_stats(void)
{
#define SYSCTL_ADD_IP_STAT(x) \
	gt_inet_add_stat("ip", #x, field_off(struct service, p_ipstat.ips_##x));
	GT_X_IP_STAT(SYSCTL_ADD_IP_STAT)
#undef SYSCTL_ADD_IP_STAT
}

static void
gt_inet_add_arp_stats(void)
{
#define SYSCTL_ADD_ARP_STAT(x) \
	gt_inet_add_stat("arp", #x, \
			 field_off(struct service, p_arpstat.arps_##x));
	GT_X_ARP_STAT(SYSCTL_ADD_ARP_STAT)
#undef SYSCTL_ADD_ARP_STAT
}

static int
gt_get_stat_api_handler(struct gt_api_conn *cp, Gt__GetStat *rq)
{
	u64 accum, *ptr;
	struct gt_inet_stat *stat, *f;
	struct service *s;
	Gt__GetStatReply *rp;

	f = NULL;
	GT_DLIST_FOREACH(stat, &gt_inet_stat_head, link) {
		if (!strcmp(stat->proto_name, rq->proto_name) &&
		    !strcmp(stat->stat_name, rq->stat_name)) {
			f = stat;
			break;
		}
	}
	...
	rp = gt_api_alloc_reply(cp, rp, get_stat);
	if (rp == NULL) {
		return -ENOMEM;
	}
	...
	return gt_api_send_reply(cp, rp, get_stat);
}

GT_API_SERVER_DEFINE_HANDLER(get_stat, gt_get_stat_api_handler);

int
gt_inet_init(void)
{
	gt_dlist_init(&gt_inet_stat_head);

	log_scope_init(&gt_main->inet_logger, "inet");

	gt_inet_add_tcp_stats();
	gt_inet_add_udp_stats();
	gt_inet_add_ip_stats();
	gt_inet_add_arp_stats();

	gt_api_register_request(&gt_main_conn, get_stat);

	gt_main->inet_cksum_offload_rx = 0;
	gt_main->inet_cksum_offload_tx = 0;
...
tcp_opts_fill(struct tcp_opts *opts, void *buf)
{
	...
	for (i = 0, ptr = buf; i < GT_ARRAY_SIZE(tcp_opt_info); ++i) {
...
tcp_opts_len(struct tcp_opts *opts)
{
	...
	for (i = 0, len = 0; i < GT_ARRAY_SIZE(tcp_opt_info); ++i) {
...
ip4_set_cksum(struct ip4_hdr *ih, void *l4h)
{
	...
	case IPPROTO_UDP:
	case IPPROTO_TCP:
		if (gt_main->inet_cksum_offload_tx) {
			ip4_cksum = 0;
			udp_cksum = 0;
		} else {
...
gt_ip4_validate_cksum(struct ip4_hdr *ih)
{
	...
	valid = 1;
	if (gt_main->inet_cksum_offload_rx == 0) {
...
tcp_opt_len(int kind)
{
	...
	for (i = 0; i < GT_ARRAY_SIZE(tcp_opt_info); ++i) {
...
tcp_input(struct in_context *in)
{
	...
	if (!gt_tcp_validate_cksum(in->in_ih, in->in_th,
				   IP4_L4_LEN(in->in_ih))) {
