...
#define DEV_FOREACH_TXRING(txr, dev) \
	for (int GT_UNIQ_VAR(i) = (dev)->dev_nmd->first_tx_ring; \
	     GT_UNIQ_VAR(i) <= (dev)->dev_nmd->last_tx_ring && \
	     ((txr = NETMAP_TXRING((dev)->dev_nmd->nifp, GT_UNIQ_VAR(i))), 1); \
	     ++GT_UNIQ_VAR(i))
...
#define DEV_FOREACH_RXRING(rxr, dev) \
	for (int GT_UNIQ_VAR(i) = (dev)->dev_nmd->first_rx_ring; \
	     GT_UNIQ_VAR(i) <= (dev)->dev_nmd->last_rx_ring && \
	     ((rxr = NETMAP_RXRING((dev)->dev_nmd->nifp, GT_UNIQ_VAR(i))), 1); \
	     ++GT_UNIQ_VAR(i))
...
static void
netmap_dev_put_tx_packet(struct dev_pkt *dev)
{
}
...
static void
netmap_dev_tx_flush(struct dev *dev)
{
}
...
netmap_dev_name(struct dev *dev, char *buf)
{
	if (dev->dev_queue_id == DEV_QUEUE_NONE) {
		...
		snprintf(buf, NETMAP_DEV_NAMSIZ, "%s%s^", NETMAP_PFX,
			 dev->dev_ifname);
	} else {
		snprintf(buf, NETMAP_DEV_NAMSIZ, "%s%s-%d", NETMAP_PFX,
...
netmap_dev_init(struct dev *dev)
{
	...
	gt_notice3(&gt_main->dev_logger, 0, "Create netmap device '%s'",
		   dev_name);
	...
	if (dev->dev_nmd != NULL) {
		...
		nmr = dev->dev_nmd->req;
		gt_notice3(&gt_main->dev_logger, 0,
			   "Netmap device '%s' created %u/%u (rx/tx) slots",
			   dev_name, nmr.nr_rx_slots, nmr.nr_tx_slots);
		return dev->dev_nmd->fd;
	} else {
		rc = -errno;
		assert(rc);
		gt_err3(&gt_main->dev_logger, -rc, "nm_open('%s') failed",
			dev_name);
		return rc;
	}
}
...
netmap_dev_deinit(struct dev *dev, bool cloexec)
{
	...
	gt_notice3(&gt_main->dev_logger, 0, "Destroy netmap device '%s'",
		   netmap_dev_name(dev, dev_name));
...
netmap_dev_rx(struct dev *dev)
{
	...
	DEV_FOREACH_RXRING(rxr, dev) {
		...
		for (i = 0; i < n; ++i) {
			...
			(*dev->dev_fn)(dev, NETMAP_BUF(rxr, slot->buf_idx),
				       slot->len);
			DEV_RXR_NEXT(rxr);
		}
	}
