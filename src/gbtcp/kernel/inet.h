...
struct eth_hdr {
	struct gt_eth_addr eh_daddr;
	struct gt_eth_addr eh_saddr;
	be16_t eh_type;
...
struct arp_ip4 {
	struct gt_eth_addr aip_sha;
	be32_t aip_sip;
	struct gt_eth_addr aip_tha;
	be32_t aip_tip;
} __attribute__((packed));
...
struct tcp_stat {
	...
	u64 tcps_states[GT_TCPS_MAX_STATES];
};
