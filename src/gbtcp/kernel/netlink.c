...
#define NLM_PUT_STR(m, attr_type, s) \
	nlm_put_attr(m, attr_type, s, strlen(s) + 1)
...
rtnl_open(unsigned int nl_groups)
{
	...
	return fd;
err:
	//gt_err3(GT_MODULE_ROUTE, -rc, "Failed to open netlink connection");
	if (fd != -1) {
		sys_close(fd);
	}
	return rc;
...
route_handle_link(struct nlmsghdr *h, struct route_msg *msg)
{
	...
	ifi = NLMSG_DATA(h);
	attr = IFLA_RTA(ifi);
	nl_get_rtattrs(attr, len, attrs, GT_ARRAY_SIZE(attrs));
	...
	if (attr != NULL) {
		...
		memcpy(msg->rtm_link.rtml_hwaddr.eth_u8, RTA_DATA(attr), 6);
	}
	return 1;
}
...
route_handle_route(struct nlmsghdr *h, struct route_msg *msg)
{
	...
	if (attr == NULL) {
		...
		memcpy(msg->rtm_route.rtmr_dst.ipa_data, RTA_DATA(attr),
		       addr_len);
	}
	...
	if (attr != NULL) {
		...
		memcpy(msg->rtm_route.rtmr_via.ipa_data, RTA_DATA(attr),
		       addr_len);
	}
	return 1;
}
...
route_msg_handler(struct nlmsghdr *h, void *udata)
{
	...
	case RTM_DELROUTE:
		...
		break;
	default:
		//gt_info3(GT_MODULE_ROUTE, 0, "Unknown netlink message type '%d'",
		//	h->nlmsg_type);
		return 0;
	}
...
rtnl_recvmsg(int fd, int (*handler)(struct nlmsghdr *, void *), void *udata)
{
	...
	if (rc < 0) {
		//if (rc != -EAGAIN) {
		//	gt_err3(GT_MODULE_ROUTE, -rc, "Unable to read netlink message");
		//}
		return rc;
	}
	len = rc;
	for (h = (struct nlmsghdr *)buf; NLMSG_OK(h, len);
		...
		case NLMSG_ERROR:
			...
			if (nlmsg_len(h) < sizeof(*err)) {
				//gt_err3(GT_MODULE_ROUTE, 0, "Netlink error truncated");
				return -EPROTO;
			}
			err = NLMSG_DATA(h);
			if (err->error) {
				//gt_err3(GT_MODULE_ROUTE, -err->error,
				//      "Receive netlink error");
				return -err->error;
			}
			break;
		case NLMSG_DONE:
		...
		default:
			...
			if (rc < 0) {
				//gt_err3(GT_MODULE_ROUTE, -rc,
				//       "Netlink '%s' message handler failed",
				//       nlmsg_type_str(h->nlmsg_type));
			} else {
				//GT_DBG(ROUTE, 0,
				//       "Netlink '%s' message processed",
				//       nlmsg_type_str(h->nlmsg_type));
			}
			if (rc < 0) {
				return rc;
	...
	if (msg.msg_flags & MSG_TRUNC) {
		//gt_err3(GT_MODULE_ROUTE, 0, "Receive truncated netlink message");
		return 0;
	}
	return 0;
}
...
route_dump(route_msg_f fn, void *udata)
{
	...
	for (i = 0; i < GT_ARRAY_SIZE(types); ++i) {
		...
		rc = gt_send_all(fd, m.nlm_buf, nlm_hdr(&m)->nlmsg_len, 0);
		if (rc < 0) {
			goto err;
		}
	...
	return 0;
err:
	//gt_err3(GT_MODULE_ROUTE, -rc, "Failed to dump route info");
	sys_close(fd);
	return rc;
}
...
err:
	if (sys_if_indextoname(ifindex, ifname) < 0) {
		//gt_err3(GT_MODULE_ROUTE, -rc, "Failed to get link (ifindex=%d) flags",
		//       ifindex);
	} else {
		//gt_err3(GT_MODULE_ROUTE, -rc, "Failed to get link '%s' flags", ifname);
	}
	sys_close(fd);
	return rc;
}
...
netlink_veth_add(const char *veth, const char *peer)
{
	...
	fd = rc;
	nlm_init(&m, buf, sizeof(buf));
	nlh_init(nlm_hdr(&m), RTM_NEWLINK,
	...
	return 0;
err:
	//gt_err3(GT_MODULE_ROUTE, -rc, "Failed to add veth interface '%s' peer '%s'", veth,
	//       peer);
	sys_close(fd);
	return rc;
}
...
netlink_link_up(int ifindex, const char *ifname, int flags)
{
	...
	return 0;
err:
	//gt_err3(GT_MODULE_ROUTE, -rc, "Failed to UP link '%s'", ifname);
	sys_close(fd);
	return rc;
}
...
netlink_link_del(const char *ifname)
{
	...
	return 0;
err:
	//gt_err3(GT_MODULE_ROUTE, -rc, "Failed to del link '%s'", ifname);
	return rc;
}
