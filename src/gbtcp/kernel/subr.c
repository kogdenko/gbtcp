...
u8 gt_freebsd_rss_key[RSS_KEY_SIZE] = {
...
int
eth_addr_aton(struct gt_eth_addr *a, const char *s)
{
	int rc;
	struct gt_eth_addr x;

	rc = sscanf(s, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx",
		    x.eth_u8 + 0, x.eth_u8 + 1, x.eth_u8 + 2, x.eth_u8 + 3,
		    x.eth_u8 + 4, x.eth_u8 + 5);
	if (rc == 6) {
		*a = x;
		return 0;
	} else {
...
int
eth_addr_is_ucast(const u8 *addr)
{
	return !eth_addr_is_mcast(addr);
}

int
eth_addr_is_bcast(const u8 *addr)
{
...
void
eth_addr_make_ip6_mcast(struct gt_eth_addr *addr, const u_char *ip6)
{
	addr->eth_u8[0] = 0x33;
	addr->eth_u8[1] = 0x33;
	addr->eth_u8[2] = ip6[12];
	addr->eth_u8[3] = ip6[13];
	addr->eth_u8[4] = ip6[14];
	addr->eth_u8[5] = ip6[15];
}
...
counter64_get(struct counter64 *c)
{
	...
	accum = 0;
	for (i = 0; i < GT_ARRAY_SIZE(c->cnt_per_service); ++i) {
...
profiler_leave(struct profiler *p)
{
	...
	if (elapsed > 3000000000ull) {
		...
		printf("profile <%-10s>: %llu tsc (%.3f%%) (%d hits)\n",
		       p->prf_name, (unsigned long long)(avg),
		       100 * ((double)p->prf_spended) / elapsed, p->prf_hits);
		p->prf_spended = 0;
		p->prf_hits = 0;
	}
	p->prf_tsc = 0;
...
u8
gt_strendswith(const char *s, const char *suf)
{
	size_t len, n;
...
char *
gt_strdup(struct gt_allocator *alc, const char *s)
{
	size_t len;

	len = strlen(s);
	return gt_memdup(alc, s, len + 1);
}

char *
gt_strndup(struct gt_allocator *alc, const char *s, size_t n)
{
	size_t len;
	char *cp;

	len = strnlen(s, n);
	cp = gt_a_malloc_align(alc, len + 1, sizeof(void *), 0);
	if (cp != NULL) {
		memcpy(cp, s, len);
		cp[len] = '\0';
	}
...
void *
gt_memdup(struct gt_allocator *alc, const void *ptr, size_t size)
{
	void *cp;

	// gt_malloc() only guarantees 4-byte alignment; duplicated structs
	// need natural alignment.
	cp = gt_a_malloc_align(alc, size, sizeof(void *), 0);
	if (cp != NULL) {
		memcpy(cp, ptr, size);
	}
	return cp;
...
#ifdef __linux__
char *
gt_get_executable_path(char *buf, size_t size)
{
	ssize_t len;
...
u32
gt_toeplitz_hash(const u_char *data, int cnt, const u8 *key, u32 key_size)
{
	u32 h, v;
	int i, b;
...
u32
gt_rss_hash4(be32_t laddr, be32_t faddr, be16_t lport, be16_t fport, u8 *key,
	     u32 key_size)
{
	int off;
	u32 h;
	...
	*(be16_t *)(data + off) = lport;
	off += 2;
	h = gt_toeplitz_hash(data, off, key, key_size);
	return h;
}

u32
gt_rss_get_queue(u32 h, u32 n_queues, u32 *indir, u32 indir_size)
{
	if (indir_size == 0) {
		return h % n_queues;
	} else {
		//		gt_dbg("indir_size=%u (h=%x)", indir_size, h);
		return indir[h & (indir_size - 1)];
	}
}

u32
gt_roundup_pow2_32(u32 x)
{
	x--;
	x |= x >> 1lu;
	x |= x >> 2lu;
...
{
	if (t < GT_NSEC_PER_SEC) {
		ts->tv_sec = 0;
		ts->tv_nsec = t;
	} else {
		ts->tv_sec = t / GT_NSEC_PER_SEC;
		ts->tv_nsec = t % GT_NSEC_PER_SEC;
	}
	return ts;
}

int
gt_connect_timed(int fd, const struct sockaddr *addr, socklen_t addrlen,
		 u64 *to)
{
	int rc, errnum, flags;
	uint64_t t, elapsed;
...
restart:
	...
	rd_nanoseconds();
	elapsed = GT_MIN(*to, nanoseconds - t);
	*to -= elapsed;
	switch (rc) {
	case 0:
		*to = 0;
	...
	case 1:
		...
		rc = sys_getsockopt(fd, SOL_SOCKET, SO_ERROR, &errnum,
				    &opt_len);
		if (rc) {
			return rc;
		}
...
out:
	if (errnum) {
		gt_err(errnum, "gt_connect_timed() failed; fd=%d, addr=%s", fd,
...
ssize_t
gt_read_timed(int fd, void *buf, size_t count, u64 *to)
{
	int flags;
	ssize_t rc;
	uint64_t t, elapsed;
...
restart:
	...
	*to -= elapsed;
	...
	fcntl_setfl_nonblock_rollback(fd, flags);
	if (rc < 0) {
		gt_err(-rc, "read_timed() failed, fd=%d", fd);
	}
	return rc;
}

#define GT_DO_ALL(func, fd, buf, count, ...) \
	({ \
		ssize_t rc, ret, off; \
\
		ret = 0; \
		for (off = 0; off < count; off += rc) { \
			rc = func(fd, (u8 *)buf + off, count - off, \
				  ##__VA_ARGS__); \
			if (rc < 0) { \
				ret = rc; \
				break; \
...
ssize_t
gt_read_all(int fd, void *buf, size_t count, u64 *to)
{
	ssize_t rc;

	rc = GT_DO_ALL(gt_read_timed, fd, buf, count, to);
	return rc;
}

ssize_t
gt_write_all(int fd, const void *buf, size_t count)
{
	ssize_t rc;

	rc = GT_DO_ALL(sys_write, fd, buf, count);
	return rc;
}

ssize_t
gt_send_all(int fd, const void *buf, size_t count, int flags)
{
	ssize_t rc;

	rc = GT_DO_ALL(sys_send, fd, buf, count, flags);
	return rc;
}

#ifdef __linux__
// TODO: pass heap for memory allocation
int
gt_read_rss_key(const char *ifname, struct gt_rss *rss,
		void *(*memdup_fn)(void *, size_t))
{
	int fd, rc, size, off;
	struct ifreq ifr;
	struct ethtool_rxfh rxfh, *rxfh2;

	memset(rss, 0, sizeof(*rss));
	rxfh2 = NULL;

	rc = fd = sys_socket(AF_INET, SOCK_DGRAM, 0);
	if (rc < 0) {
		goto err;
	}
	...
	size = (sizeof(rxfh) + rxfh.key_size +
		rxfh.indir_size * sizeof(rxfh.rss_config[0]));
	rxfh2 = malloc(size);
	if (rxfh2 == NULL) {
		rc = -ENOMEM;
		goto err;
	...
	rxfh2->cmd = ETHTOOL_GRSSH;
	rxfh2->indir_size = rxfh.indir_size;
	rxfh2->key_size = rxfh.key_size;
	ifr.ifr_data = (void *)rxfh2;
	rc = sys_ioctl(fd, SIOCETHTOOL, (uintptr_t)&ifr);
	if (rc < 0) {
		goto err;
	}

	assert(sizeof(*rss->rss_indir_table) == sizeof(rxfh2->rss_config[0]));
	off = rxfh2->indir_size * sizeof(u32);
	rss->rss_indir_table_size = rxfh2->indir_size;
	rss->rss_indir_table = (*memdup_fn)((u8 *)rxfh2->rss_config, off);
	if (rss->rss_indir_table == NULL) {
		goto err;
	}
	rss->rss_key_size = rxfh2->key_size;
	rss->rss_key =
		(*memdup_fn)((u8 *)rxfh2->rss_config + off, rss->rss_key_size);
	if (rss->rss_key == NULL) {
		//		shm_free(rss->rss_indir_table);
		goto err;
	}
...
err:
	...
	rss->rss_key_size = 0;
	rss->rss_indir_table_size = 0;
	return rc;
}

int
gt_get_ifchannels(const char *ifname)
{
	int rc, fd;
	struct ifreq req;
...
err:
	gt_err(-rc, "Failed to get ETHTOOL_GCHANNELS; ifname\"%s\"", ifname);
	sys_close(fd);
	return rc;
}
...
err:
	gt_err(0, "/proc/%d/status has invalid format", pid);
	return -EPROTO;
}*/

int
gt_get_ifhwaddr(struct gt_eth_addr *hwaddr, u32 *flags, const char *ifname)
{
	int rc, fd;
	struct ifreq ifr;
	...
	rc = sys_ioctl(fd, SIOCGIFHWADDR, (uintptr_t)(void *)&ifr);
	close(fd);
	if (rc) {
		return rc;
	}

	memcpy(hwaddr->eth_u8, ifr.ifr_hwaddr.sa_data, sizeof(hwaddr->eth_u8));
	// TODO: Get flags
	*flags = IFF_UP;
...
gt_get_ifchannels(const char *ifname)
{
	// TODO: Get number of channels (use netmap)
	return 1;
}
...
{
	memcpy(rss_key, gt_freebsd_rss_key, RSS_KEY_SIZE);
	return 0;
}
...
read_proc_comm(char *name, int pid)
{
	...
	gt_strzcpy(name, info->ki_comm, SERVICE_COMM_MAX);
	free(info);
	return 0;
}*/
...
gt_get_ifhwaddr(struct gt_eth_addr *hwaddr, u32 *flags, const char *ifname)
{
	...
	for (ifa = ifap; ifa != NULL; ifa = ifa->ifa_next) {
		if (ifa->ifa_addr->sa_family == AF_LINK &&
			...
			memcpy(hwaddr->eth_u8, LLADDR(addr), 6);
			*flags = ifa->ifa_flags;
			rc = 0;
...
gt_set_affinity(int cpu_id)
{
	...
	if (rc != 0) {
		gt_err(rc, "pthread_setaffinity_np(self, %d) failed", cpu_id);
	}
	return -rc;
}
