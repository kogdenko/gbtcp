...
strbuf_remove(struct strbuf *sb, int pos, int len)
{
	...
	memmove(sb->sb_buf + pos, sb->sb_buf + pos + len,
		sb->sb_len - (pos + len));
	sb->sb_len -= len;
}
...
strbuf_insert(struct strbuf *sb, int pos, const void *data, int len)
{
	...
	if (pos >= sb->sb_cap) {
		...
		memmove(sb->sb_buf + pos + len, sb->sb_buf + pos,
...
void
strbuf_add_eth_addr(struct strbuf *sb, struct gt_eth_addr *a)
{
	strbuf_addf(sb, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx",
		    a->eth_u8[0], a->eth_u8[1], a->eth_u8[2], a->eth_u8[3],
		    a->eth_u8[4], a->eth_u8[5]);
}
