...
u8 gt_dlist_is_empty(struct gt_dlist *);
...
#define GT_DLIST_REMOVE(var, field) gt_dlist_remove(&(var)->field)

#define gt_dlist_foreach(var, head) \
	for (var = (head)->dls_next; var != (head); var = var->dls_next)

#define gt_dlist_forrach_reverse(var, head) \
	for (var = (head)->dls_prev; var != (head); var = var->dls_prev)

#define gt_dlist_foreach_rcu(var, head) \
	for (var = READ_ONCE((head)->dls_next); var != (head); \
	     var = READ_ONCE(var->dls_next))
...
#define GT_DLIST_FOREACH_REVERSE(var, head, field) \
	for (var = GT_DLIST_LAST(head, typeof(*(var)), field); \
	     &((var)->field) != (head); var = GT_DLIST_PREV(var, field))

#define GT_DLIST_FOREACH_RCU(var, head, field) \
	for (var = gt_dlist_entry_rcu((head)->dls_next, typeof(*(var)), \
				      field); \
	     &var->field != (head); \
	     var = gt_dlist_entry_rcu(var->field.dls_next, typeof(*(var)), \
				      field))
...
#define GT_DLIST_FOREACH_SAFE(var, head, field, tvar) \
	for (var = GT_DLIST_FIRST(head, typeof(*(var)), field); \
	     (&((var)->field) != (head)) && \
	     ((tvar = GT_DLIST_NEXT(var, field)), 1); \
	     var = tvar)

#define GT_DLIST_FOREACH_REVERSE_SAFE(var, head, field, tvar) \
	for (var = GT_DLIST_LAST(head, typeof(*(var)), field); \
	     (&((var)->field) != (head)) && \
	     ((tvar = GT_DLIST_PREV(var, field)), 1); \
	     var = tvar)
